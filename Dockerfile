...
# El front controller vive en SGDM/publico/index.php y el routing depende
# del .htaccess (mod_rewrite + mod_headers). Las tres capas del MVC
# (SGDM/controlador, SGDM/modelo, SGDM/vista) quedan fuera del DocumentRoot.
FROM php:8.2-apache

# Extensiones PHP:
#   - pdo_mysql: conexión a la base de datos (SGDM/modelo/Conexion.php).
...
ENV APACHE_DOCUMENT_ROOT=/var/www/html/SGDM/publico
...
# almacenamiento/throttle debe ser escribible por Apache (fuerza bruta);
...
RUN mkdir -p /var/www/html/SGDM/almacenamiento/throttle \
    && chown -R www-data:www-data /var/www/html/SGDM/almacenamiento
