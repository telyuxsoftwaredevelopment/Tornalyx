...
-- ──────────────────────────────────────────────────────────────
-- Columnas de aprobación por email: si doc_acceso ya existía sin ellas, se
-- agregan.
--
-- OJO con la idempotencia acá: `ADD COLUMN IF NOT EXISTS` es sintaxis de
-- MariaDB y MySQL 8 la rechaza con un error 1064 (error de sintaxis), así
-- que en MySQL —el motor que usan docker-compose y el servidor— estas tres
-- sentencias NUNCA se aplicaban y las columnas de aprobación quedaban sin
-- crear en cualquier base que ya tuviera doc_acceso de antes.
--
-- Se escriben sin IF NOT EXISTS a propósito: al re-ejecutar la migración
-- devuelven 1060 (columna duplicada) y 1061 (índice duplicado), dos de los
-- códigos que tanto el migrador PHP (modelo/Migracion.php) como
-- scripts/desplegar.sh tratan como "esto ya estaba hecho" y siguen de largo.
-- ──────────────────────────────────────────────────────────────
ALTER TABLE doc_acceso ADD COLUMN aprob_token_hash CHAR(64)  NULL DEFAULT NULL;
ALTER TABLE doc_acceso ADD COLUMN aprob_token_exp  TIMESTAMP NULL DEFAULT NULL;
ALTER TABLE doc_acceso ADD INDEX idx_aprob_token (aprob_token_hash);
