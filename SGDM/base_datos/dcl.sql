...
--        - tornalyx_dml -> DB_PASS del .env de la app (SGDM/modelo/Conexion.php).
...
-- (SGDM/base_datos/migrations/*.sql) sobre tornalyx_db. Lo usa únicamente
...
-- Una migración no es solo estructura, y con DDL a secas desplegar.sh no
-- puede terminar su trabajo. Le hacen falta también:
--   · schema_migrations: cada add_*.sql se registra a sí misma con un
--     INSERT, y desplegar.sh lee esa tabla para saber qué falta correr y
--     anota ahí el motivo cuando una migración queda a medio aplicar.
--   · las tablas de negocio: varias migraciones adaptan los datos viejos al
--     esquema nuevo antes de cambiarlo (ver el UPDATE de
--     add_rol_torneo_simplificado.sql, que pasa las cuentas 'organizador' a
--     'participante' antes de reducir el ENUM). Sin ese UPDATE el ALTER
--     falla por dato truncado y la migración queda por la mitad.
--
-- No debilita el modelo: quien ya tiene ALTER y DROP sobre una tabla puede
-- destruir sus datos igual, así que negarle el UPDATE no protege nada y sí
-- rompe el despliegue. Lo que sigue siendo cierto —y es lo que importa— es
-- que el usuario de la app en runtime (tornalyx_dml) NO puede tocar la
-- estructura, y que estas credenciales no viven en el .env de la app.
GRANT SELECT, INSERT, UPDATE, DELETE ON tornalyx_db.* TO 'tornalyx_ddl'@'localhost';
...
-- (SGDM/modelo/Conexion.php). Solo CRUD sobre las tablas de negocio, sin
