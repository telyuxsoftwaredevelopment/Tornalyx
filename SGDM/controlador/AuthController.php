...
require_once __DIR__ . '/../modelo/OtpCode.php';
require_once __DIR__ . '/../comun/Session.php';
require_once __DIR__ . '/../comun/LoginThrottle.php';
require_once __DIR__ . '/../comun/Mailer.php';
...
class AuthController extends Controller {
    ...
    private function iniciarMfaEmail(array $usuario): void {
        ...
        // Sin transporte de correo configurado no hay forma de entregar el
        // código, y el 2FA obligatorio deja el sistema entero sin acceso: no
        // se puede entrar ni siquiera como administrador. En desarrollo eso
        // convierte una instalación limpia en algo inusable, así que ahí se
        // omite el segundo factor; en cualquier otro entorno se mantiene
        // obligatorio y se explica qué falta configurar.
        if (!(new Mailer())->isConfigured()) {
            if (getenv('APP_ENV') === 'development') {
                error_log('MFA omitido: APP_ENV=development y no hay SMTP/Brevo configurado.');
                session_regenerate_id(true);
                Session::login($usuario);
                $this->jsonSuccess([
                    'rol'   => $usuario['rol'],
                    'aviso' => 'Modo desarrollo: se omitió la verificación en dos pasos porque no hay correo configurado.',
                ]);
                return;
            }
            error_log('MFA imposible: no hay BREVO_API_KEY ni credenciales SMTP configuradas.');
            http_response_code(500);
            $this->jsonError('El envío de códigos de verificación no está configurado. Avisá al administrador del sistema.');
            return;
        }
